# =============================================================================
#  Плагин: Информация о пользователе (Максимальная версия)
#  Версия: 4.0.0
#  Категория: utilities
# =============================================================================
...
class TelegramUserFinder:
    ...
    async def find_by_username(self, username: str, client) -> Dict:
        """Поиск по username"""
    ...
    async def find_by_phone(self, phone: str, client) -> Dict:
    ...
    async def _collect_user_info(self, user: User, full_user: UserFull, client) -> Dict:
        info = {
            "id": user.id,
    ...
    async def _get_status_text(self, user: User) -> str:
    ...
    async def _get_last_seen(self, user: User) -> str:
        if not hasattr(user, 'status'):
            return "Неизвестно"
        status = user.status
...
async def _format_user_info(user: User, full_user: UserFull, event) -> str:
...
async def _get_status_text(user: User) -> str:
    if not hasattr(user, 'status'):
        return "Неизвестно"
    status = user.status
    if isinstance(status, UserStatusOnline):
