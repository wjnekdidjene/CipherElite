...
def init(client_instance):
    commands = [
        ...
        ".afkclear - Очистить статистику AFK"
    ]
...
• 🔔 Упоминания
• 🎨 Случайные цитаты
• 🛡 Защита от спама (5 запросов/мин)

**Примеры:**
• `.afk` - Без причины
• `.afk Обедаю` - С причиной
• `.afk На встрече` - С причиной"""
        await event.reply(help_text)

    @CipherElite.on(events.NewMessage(pattern=r"\.afkclear"))
    @rishabh()
    async def afkclear_handler(event):
        try:
            user_id = event.sender_id
            if user_id in afk_stats:
                del afk_stats[user_id]
                await event.reply("🗑 **Статистика AFK очищена!**")
            else:
                await event.reply("📭 **У вас нет статистики AFK.**")
        except Exception as e:
            logger.error(f"Ошибка AFKCLEAR: {e}")
            await event.reply(f"❌ Ошибка: {str(e)}")
