...
from utils.utils import init_client, set_bot, get_client
    ...
    for plugin_name in plugins:
        try:
            ...
            if hasattr(module, "init_bot_plugin"):
                # ✅ ПЕРЕДАЁМ bot_client, owner_id, owner_name
                if asyncio.iscoroutinefunction(module.init_bot_plugin):
                    await module.init_bot_plugin(bot_client, owner.id, owner_name)
                else:
                    module.init_bot_plugin(bot_client, owner.id, owner_name)
                loaded.append(plugin_name.split(".")[-1])
...
async def start_bot(bot_instance):
    ...
    try:
        ...
        init_client(client)  # ✅ ОБЯЗАТЕЛЬНО!
