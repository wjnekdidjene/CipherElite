...
from vars import config as vars_config  # ✅ ИСПРАВЛЕНО
...
async def load_bot_plugins(bot_client, user_client):
    ...
    Config.OWNER_ID = owner_id  # ✅ ТЕПЕРЬ РАБОТАЕТ
...
async def start_bot(bot_instance):
    load_translations(vars_config.LANGUAGE)  # ✅ ИСПРАВЛЕНО

    banner("CipherElite", "2.0.0", "Rishabh Anand", "@thanosceo")

    if not vars_config.API_ID or not vars_config.API_HASH or not vars_config.ELITE_SESSION:  # ✅ ИСПРАВЛЕНО
        raise ValueError("❌ API_ID, API_HASH и ELITE_SESSION обязательны!")

    start = step("Подключение к Telegram")
    try:
        session = thanos_protect(vars_config.ELITE_SESSION)  # ✅ ИСПРАВЛЕНО
        client = TelegramClient(StringSession(session), vars_config.API_ID, vars_config.API_HASH)  # ✅ ИСПРАВЛЕНО
        await client.start()
    ...
    box("CIPHER ELITE", [
        ...
        ("Язык", vars_config.LANGUAGE.upper()),  # ✅ ИСПРАВЛЕНО
        ("База", db_type)
    ])
